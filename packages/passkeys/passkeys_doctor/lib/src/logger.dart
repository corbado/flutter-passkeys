...
class Logger {
  ...
  Future<void> _printException(PlatformException error, List<Checkpoint> checkpointList) async{
    print('Latest Exception:');

    switch (error.code) {
      case 'cancelled':
        print(' PasskeyAuthCancelledException: Thrown when the user cancels the passkey flow.');
      ...
      case 'exclude-credentials-match':
        print(' ExcludeCredentialsCanNotBeRegisteredException: Thrown when the user tries to register but one of the excluded credentials matches an existing one.');
      ...
      case 'android-missing-google-sign-in':
        print(' MissingGoogleSignInException: Thrown when the user tries to register or authenticate but isn’t signed in to a Google account on their device.');
        print(' Platforms: Android');
        print(' Suggestions:');
        print('  • Ask the user to sign in to their Google account first.');
        print('  • Use a fallback method (e.g. login via email OTP).');
      ...
      case 'android-sync-account-not-available':
        ...
        print(' Platforms: Android');
        print(' Suggestions:');
        print('  • Make sure the emulator has Google Play Services integrated.');
        print('  • Cold-boot your emulator and ensure the user is signed in to Google.');
        break;
      ...
      case 'domain-not-associated':
        ...
        print(' DomainNotAssociatedException: Thrown when your app’s domain isn’t correctly associated (no Digital Asset Links or AASA file).');
        print(' Platforms: Android, iOS');
        print(' Suggestions:');
        print('  • Verify your domain association setup (assetlinks.json / apple-app-site-association).');
        break;

      case 'deviceNotSupported':
        print(' DeviceNotSupportedException: Thrown when the device does not support passkeys.');
        print(' Platforms: Android, iOS');
        print(' Suggestions:');
      ...
      case 'android-no-credential':
        print(' NoCredentialsAvailableException: Thrown when the user has no credentials available for the authenticator.');
        print(' Platforms: Android, iOS');
        print(' Suggestions:');
        print('  • Use a fallback method (e.g. login via email OTP).');
        break;
      ...
      case 'android-no-create-option':
        print(' NoCreateOptionException: Thrown when no viable creation options were found for the given CreateCredentialRequest.');
        print(' Platforms: Android');
        print(' Suggestions:');
        print('  • Use a fallback method (e.g. redirect the user to the device settings).');
        print('  • Ask the user to enable passkeys in the device settings.');
        break;

      case 'malformed-base64-url-challenge':
        print(' MalformedBase64UrlChallenge: Thrown when the challenge is not a valid Base64URL-encoded string.');
        print(' Platforms: Android, iOS, Web');
        print(' Suggestions:');
        print('  • Make sure that the challenge is a valid Base64URL-encoded string without padding.');
        break;

      case 'malformed-base64-url-credential-id':
        print(' MalformedBase64UrlCredentialID: Thrown when a credential ID is not a valid Base64URL-encoded string.');
        print(' Platforms: Android, iOS, Web');
        print(' Suggestions:');
        print('  • Make sure that the credential ID is a valid Base64URL-encoded string without padding.');
        break;

      case 'malformed-base64-url-user-id':
        print(' MalformedBase64UrlUserID: Thrown when the user ID is not a valid Base64URL-encoded string.');
        print(' Platforms: Android, iOS, Web');
        print(' Suggestions:');
        print('  • Make sure that the user ID is a valid Base64URL-encoded string.');
        break;

      default:
        if (error.code.startsWith('android-unhandled') || error.code.startsWith('ios-unhandled')) {
          print(' UnhandledAuthenticatorException: Thrown when an authenticator-thrown exception isn’t handled by this package.');
          print(' Platforms: Android, iOS');
          print(' Suggestions:');
          print('  • Report this exception to the package authors (code: ${error.code}).');
        }
    }
  }
}