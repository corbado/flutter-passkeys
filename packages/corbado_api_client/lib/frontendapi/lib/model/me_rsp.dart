...
class MeRsp {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key),
              'Required key "MeRsp[$key]" is missing from JSON.');
          assert(json[key] != null,
              'Required key "MeRsp[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<MeRsp> listFromJson(
    dynamic json, {
    bool growable = false,
  }) {
    final result = <MeRsp>[];
  ...
  static Map<String, List<MeRsp>> mapListFromJson(
    dynamic json, {
    bool growable = false,
  }) {
    ...
    if (json is Map && json.isNotEmpty) {
      ...
      for (final entry in json.entries) {
        map[entry.key] = MeRsp.listFromJson(
          entry.value,
          growable: growable,
        );
      }
